...
- Returns whatever the method returns:
  - return a **String** → plain text (like `/hello` right now)
  - return an **object** → Spring auto-converts to **JSON** (via Jackson) — coming in Part 4.
...
## Next up
- **Part 4:** return proper JSON using a model class.
