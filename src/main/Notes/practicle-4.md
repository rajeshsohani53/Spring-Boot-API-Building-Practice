...
- Fixed the JS to check `response.ok` before showing success (see Part 6).

---

## Part 6 — Full CRUD (UPDATE, DELETE, GET-by-id)

Completed all four CRUD operations on the `/students` resource. Two new concepts: `@PathVariable`, and `@PutMapping` / `@DeleteMapping`.
...
- URL `/students/{id}` has `{id}` as a placeholder. Hit `/students/5` → Spring passes `5` into the method.
- Difference from `@RequestBody`:
  - `@PathVariable` → data comes from the **URL path** (`/students/5`) → identifies WHICH resource.
  - `@RequestBody` → data comes from the **request body** (JSON) → the actual data (POST/PUT).

**PUT uses BOTH together** — the standard REST update pattern:
- `@PathVariable id` = WHICH student (from URL)
- `@RequestBody updated` = WHAT to change it to (from body)
- "How do you update a resource in REST?" → PUT to `/resource/{id}` with new data in the body.

### The full CRUD map — THIS IS REST
| Operation | HTTP verb | URL | Data source |
|-----------|-----------|-----|-------------|
| Create | POST | `/students` | `@RequestBody` |
| Read all | GET | `/students` | — |
| Read one | GET | `/students/{id}` | `@PathVariable` |
| Update | PUT | `/students/{id}` | `@PathVariable` + `@RequestBody` |
| Delete | DELETE | `/students/{id}` | `@PathVariable` |

- Same resource, five endpoints, distinguished by **verb** + **path pattern**.
- `/students` and `/students/{id}` don't clash: different path patterns. Within the same path, the **verb** separates them. Path pattern + verb = unique endpoint.
- `removeIf(...)` = clean Java 8 way to delete from a list by a condition.

### Frontend for CRUD
- Upgraded `list.html`: each row has per-row **Update** (inputs + button → `PUT /students/{id}`) and **Delete** (button → `DELETE /students/{id}`) controls. The list IS the control panel (better than separate update/delete pages that force typing ids).
- Fixed `add.html` to check `response.ok` before showing "Saved successfully" (fixes the fake-success bug from Part 5b).
- Client side of `@PathVariable`: the JS uses `"/students/" + id` in the URL — id goes in the PATH, matching `/students/{id}`. Update also sends a body = the `@PathVariable` + `@RequestBody` combo, seen from the frontend.

### Mindset note
- "First make it work, then make it right, then make it fast." Don't chase perfect design at the learning stage — ship the working thing, understand it, refine later.
